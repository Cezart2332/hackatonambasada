...
export function AuthScreen({
  ...
  async function signInWithGoogle() {
    ...
    try {
      ...
      if (result.error) {
        throw new Error(result.error.message || "Autentificare Google eșuată.");
      }
    } catch (error) {
      setAuthError(messageFromUnknownError(error, "Autentificare Google eșuată."));
      setAuthLoading(false);
    }
  }
                  ...
                  <form onSubmit={submitLogin} className="space-y-4">
                    ...
                    <div className="relative py-1">
                      <div className="absolute inset-0 flex items-center">
                        <span className="w-full border-t border-[#ded5bf]" />
                      </div>
                      <div className="relative flex justify-center text-xs uppercase">
                        <span className="bg-[#fffdf7] px-2 text-[#8a9582]">sau</span>
                      </div>
                    </div>
                    <Button
                      type="button"
